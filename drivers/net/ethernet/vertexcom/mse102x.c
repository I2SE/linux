...
static int
mse102x_info_open(struct inode *inode, struct file *file)
{
	return single_open(file, mse102x_info_show, inode->i_private);
}

static const struct file_operations mse102x_info_ops = {
	.open = mse102x_info_open,
	.read = seq_read,
	.llseek = seq_lseek,
...
{
	__be16 *header = (__be16 *)skb_push(skb, DET_SOF_LEN);
...
{
	__be16 *footer = (__be16 *)skb_put(skb, DET_DFT_LEN);
...
static int mse102x_probe_spi(struct spi_device *spi)
{
	...
	ndev = alloc_etherdev(sizeof(struct mse102x_net_spi));
	if (!ndev)
		return -ENOMEM;
